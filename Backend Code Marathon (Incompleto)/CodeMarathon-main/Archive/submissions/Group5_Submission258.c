...
void alterarEstadoCarro (char ma[10]) {
    ...
    for (int i = 0; i< 5; i++) {
        if (parqueArtur.Carros[i] != NULL && strcmp(ma, parqueArtur.Carros[i]->matricula) == 0 && parqueArtur.Carros[i]->EmUso) {
...
int main(int argc, const char * argv[]) {
    ...
    for (int i = 0; i < 5; i++) {
        if (parqueArtur.Carros[i] != NULL && parqueArtur.Carros[i]->EmUso)

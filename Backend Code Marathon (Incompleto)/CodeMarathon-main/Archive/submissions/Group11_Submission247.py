...
class ParqueEstacionamento:
    ...
    def aceitarCarro(self, carro):
        if self.checkCarInPark(carro) in self.carros[True]:
            self.alterarEstadoCarro(carro)
        else:
            self.counter += 1
...
parque.carros[False].sort(key = lambda x: x.admission_num)
